...
# Styling CSS Dark Fintech - Palette Frost Indigo con Contrasto Alto & FIX SAFARI IOS
st.markdown(
    """
    <style>
...
LEAGUES_CONFIG = {
    ...
    "Champions League": {"key": "soccer_uefa_champs_league", "has_players": False},
    "Europa League": {"key": "soccer_uefa_europa_league", "has_players": False},
    "Conference League": {"key": "soccer_uefa_europa_conference_league", "has_players": False},
    "Campionati Europei": {"key": "soccer_uefa_european_championship", "has_players": False},
    "Campionati del Mondo": {"key": "soccer_fifa_world_cup", "has_players": False}
}
...
TEAM_METRICS = {
    "Inter": {"gf_h": 2.25, "gf_a": 1.90, "ga_h": 0.65, "ga_a": 0.80, "xg_5": 2.15, "xg_s": 2.05, "sot_pro": 6.2, "sot_against": 3.1, "corners_pro": 6.4, "corners_against": 3.6, "cross": 21.5, "blocked_shots": 5.4, "fouls_pro": 11.2, "fouls_against": 12.8, "cards_avg": 1.8, "modulo": "3-5-2", "stile": "Pressing Alto & Sovrapposizione Fasce", "possesso": 61.2},
    ...
    "Milan": {"gf_h": 2.05, "gf_a": 1.65, "ga_h": 1.10, "ga_a": 1.25, "xg_5": 1.90, "xg_s": 1.80, "sot_pro": 5.6, "sot_against": 4.4, "corners_pro": 5.8, "corners_against": 4.2, "cross": 19.5, "blocked_shots": 5.2, "fouls_pro": 11.8, "fouls_against": 12.0, "cards_avg": 2.3, "modulo": "4-2-3-1", "stile": "Transizione Rapida & Spinta Esterne", "possesso": 56.0},
    ...
    "Parma": {"gf_h": 1.35, "gf_a": 1.10, "ga_h": 1.45, "ga_a": 1.65, "xg_5": 1.30, "xg_s": 1.25, "sot_pro": 4.2, "sot_against": 5.4, "corners_pro": 4.7, "corners_against": 5.8, "cross": 15.5, "blocked_shots": 3.7, "fouls_pro": 13.5, "fouls_against": 11.5, "cards_avg": 2.2, "modulo": "4-2-3-1", "stile": "Contropiede Diretto ad Alta Velocita", "possesso": 45.2},
...
# Rose Complete Serie A con metriche P90
...
class MatchAnalystEngine:
    ...
    def analyze_player_fouls(player, opp_team, ref_data, line=1.5, min_edge=0.015, injuries_df=None):
        ...
        return {
            ...
            "note": f"Arbitro: {ref_data.get('name')} (Media: {ref_data.get('fouls_avg'):.1f} falli/gara) | Falli subiti avversario: {opp_met['fouls_against']:.1f}"
        }
...
# Filtro Turno Singolo con Fallback Sicuro
def filter_current_matchday(matches):
    if not matches:
        # Fallback automatico turno Serie A
        now_dt = datetime.datetime.now()
        start_label = now_dt.strftime("%d/%m/%Y")
        end_label = (now_dt + datetime.timedelta(days=3)).strftime("%d/%m/%Y")
        mock_matches = [
            ...
            {"home_team": "Roma", "away_team": "Lazio", "commence_time": now_dt.isoformat()},
            {"home_team": "Atalanta", "away_team": "Fiorentina", "commence_time": now_dt.isoformat()},
            {"home_team": "Bologna", "away_team": "Torino", "commence_time": now_dt.isoformat()},
            {"home_team": "Venezia", "away_team": "Cagliari", "commence_time": now_dt.isoformat()}
        ]
        return mock_matches, start_label, end_label
...
# BOX RAPIDO DI SBLOCCO VIP PER SMARTPHONE / SAFARI
if not is_premium:
    with st.expander("🔓 SBLOCCO RAPIDO PIANO PRO (Inserisci Codice VIP)", expanded=True):
        c_m1, c_m2 = st.columns([3, 1])
        with c_m1:
            mob_code = st.text_input("Codice Promozionale VIP / Tester", placeholder="es. Valuebet2026", type="password", key="mob_vip_in")
        with c_m2:
            st.write("")
            st.write("")
...
with tab_scan:
    ...
    with st.expander("Guida ai Termini & Legenda Quantitativa", expanded=False):
        st.markdown("""
        * **Probabilita Modello:** La percentuale reale stimata dal nostro algoritmo matematico che l'evento si verifichi.
        * **Quota Equa:** Il prezzo puro matematico dell'evento ($1 / \\text{Probabilita}$), calcolato senza margini o commissioni del bookmaker.
        * **Quota Minima (Valore):** Il prezzo minimo a cui conviene entrare sul mercato. Se il tuo bookmaker offre una quota pari o superiore a questo valore, la giocata ha un vantaggio matematico (Value Bet). Sotto questa quota e **NO BET**.
...
with tab1:
    ...
    st.caption("Classifica delle migliori 5 opportunita matematiche sui mercati live scaricati via API.")
    if matches:
        cat1_all = []
        for m in matches:
            h = clean_name(m.get("home_team", ""))
            ...
            avg_ov = float(np.mean(ov_odds_list)) if ov_odds_list else round(1.0 / p_ov25 * 1.05, 2)
            ...
            avg_un = float(np.mean(un_odds_list)) if un_odds_list else round(1.0 / p_un25 * 1.05, 2)
...
if is_serie_a:
    ...
    with tab_inj:
        ...
        st.caption("Inserisci qui settimanalmente i calciatori infortunati. L'algoritmo ricalcolera istantaneamente il peso su xG e linee statistiche.")
