...
user_pwd = st.sidebar.text_input("Password d'accesso", type="password")

if app_pwd and user_pwd != app_pwd:
  st.warning(
      "Inserisci la password corretta nella barra laterale per accedere"
      " all'analisi e alle quote."
  )
  st.stop()
...
with tab1:
  ...
  if fetch_btn or "api_data_cache" not in st.session_state:
    if user_api_key:
      ...
      st.warning(
          "Chiave API non configurata nei Secrets di Streamlit."
      )
